...
DELIMITER $$
CREATE FUNCTION fn_02() RETURNS INT
BEGIN
...
--     LIMIT 1;
    ...
    SELECT type_id INTO v_type_id
    FROM c4
    WHERE question_count = (SELECT MAX(question_count)
							FROM c4);
...
BEGIN
    ...
    UNION
    SELECT 'username' AS type,username AS name
	FROM account
...
BEGIN
    ...
    SELECT SUBTRING_INDEX(in_email,"@",1) INTO v_username;
    
    SELECT position_id INTO v_position_id
    FROM position
    WHERE department_name = "Phòng chờ";
    
    INSERT INTO account (email	 , username 	, Full_name		, department_id		)
	VALUES				(in_email, v_username	, in_full_name	, v_department_id	);
...
DELIMITER $$
CREATE PROCEDURE sp_08(IN in_type_name ENUM("Esaay","Multiple-Choice"))
BEGIN
		WITH c1 AS(
...
SELECT month, COUNT(question_id) AS question_coutn
FROM c1
